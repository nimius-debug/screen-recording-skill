...
// Newest /opt/pw-browsers/<prefix>-<build>/<...>/<binary> that exists, or null.
...
export async function resolveFfmpeg() {
  ...
  try {
    const mod = await importDefault('ffmpeg-static');
    if (mod) candidates.push(mod);
  } catch (_) {
    /* ignore */
  }
  for (const bin of candidates) {
    const probe = spawnSync(bin, ['-hide_banner', '-encoders'], {
      encoding: 'utf8',
    });
...
// Dynamic default import that tolerates absence.
